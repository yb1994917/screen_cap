...
public class MiniCapUtil implements ScreenSubject {
	...
	private static final int TOUCH_PORT = 1312;
	...
	private int sizeW = 1024;
	private int sizeH = 768;
	...
	public boolean isSupoort(){
		String supportCommand = String.format("LD_LIBRARY_PATH=/data/local/tmp /data/local/tmp/minicap -P %s@%s/0 -t", "1024x768","720x1280");
	...
	private void init() {
		...
		File minicapBinFile = null;
		File minitouchBinFile = null;
		File minicapSoFile = null;
		try {
			minicapBinFile = Constant.getFileFromJar("minicap/bin/"+abi +"/"+ MINICAP_FILE);
			minitouchBinFile = Constant.getFileFromJar("minitouch/"+ abi + "/" + MINITOUCH_FILE);
			minicapSoFile = Constant.getFileFromJar("minicap/shared/"+ "android-" + sdk + "/" + abi + "/" + MINICAP_SO);
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.out.println("minicapBinFile"+minicapBinFile.getAbsolutePath() +" exist:"+minicapBinFile.exists() +" len:"+minicapBinFile.length());
		System.out.println("minitouchBinFile"+minitouchBinFile.getAbsolutePath()+" exist:"+minitouchBinFile.exists() +" len:"+minitouchBinFile.length());
		System.out.println("minicapSoFile"+minicapSoFile.getAbsolutePath()+" exist:"+minicapSoFile.exists() +" len:"+minicapSoFile.length());
		try {
			...
			System.out.println("vm size ret:"+output);
	...
	public void takeScreenShotOnce() {
		...
		String takeScreenShotCommand = String.format(
				MINICAP_TAKESCREENSHOT_COMMAND, MINICAP_FILE,"1024x768", "360x640", savePath);
...
//			}).start();
		...
		public void run() {
			...
			try {
				...
				final String startCommand = String.format(
						MINICAP_START_COMMAND, MINICAP_FILE,"1024x768", "720x1280");
	...
	class TouchThread implements Runnable {
		...
		public void run() {
			...
			System.out.println("TouchThread" + startCmd);
